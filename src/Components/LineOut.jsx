...
function WorkOrder() {
  ...
  // ================= TOOL HANDLERS =================
  const addToolRow = () => {
    setTools([...tools, { tool_id: "", category_id: "", tool_qty: "" }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const validTools = tools.filter(
      (t) => t.tool_id && t.category_id && t.tool_qty
    );

    if (!workDate || !machineId || !workerId || validTools.length === 0) {
      alert("Please fill all required fields and at least one valid tool");
      return;
    }
    ...
    try {
      ...
      if (res.data.status === 1) {
        ...
        setMachineId("");
        setWorkerId("");
        setTools([{ tool_id: "", category_id: "", tool_qty: "" }]);
          ...
          {tools.map((tool, index) => (
            <div key={index} className="cs-form" style={{ marginBottom: "10px" }}>
              ...
              <select
                value={tool.tool_id}
                onChange={(e) =>
                  handleToolChange(index, "tool_id", e.target.value)
                }
              >
                <option value="">Select Tool</option>
                {toolOptions.map((t) => (
              ...
              <select
                ...
                onChange={(e) =>
                  handleToolChange(index, "category_id", e.target.value)
                }
              >
                <option value="">Select Category</option>
